...
def register_routes(app):
    ...
    def add_student():
        ...
        return render_template("student_form.html", student=None, programs=programs, statuses=statuses)
    ...
    def edit_student(student_id):
        ...
        return render_template("student_form.html", student=student, programs=programs, statuses=statuses)
    ...
    def add_submission():
        ...
        if request.method == "POST":
            try:
                ...
                # A small validation step before the transaction starts.
                existing_enrollment = Enrollment.query.filter_by(
                    student_id=student_id,
                    course_id=assignment.course_id,
                ).first()
                if not existing_enrollment:
                    raise ValueError("Student must be enrolled in the course before a submission can be recorded.")

                score_value = to_score(request.form.get("score"), assignment.max_score)
                note_text = (request.form.get("grader_note") or "").strip()

                # Transaction logic required by the rubric:
                # 1) create the submission
                # 2) create an activity log row
                ...
                with db.session.begin():
                    ...
                    db.session.add(
                        ActivityLog(
                            ...
                            event_note=f"Submission stored for student #{student_id} on assignment #{assignment_id}.",
                        )
                    )
        ...
        return render_template("submission_form.html", assignments=assignments, students=students)
